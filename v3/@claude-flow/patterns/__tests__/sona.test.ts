 ...
 *
 * Tests for Pattern integration with @ruvector/sona package.
...
import type { Trajectory, PatternMode, PatternModeConfig } from '../src/types.js';
...
describe('PatternLearningEngine', () => {
  ...
  let modeConfig: PatternModeConfig;
  ...
  beforeEach(() => {
    ...
    engine = new PatternLearningEngine('balanced', modeConfig);
  });
  ...
  describe('Initialization', () => {
    ...
    it('should initialize with real-time mode', () => {
      const rtEngine = createPatternLearningEngine('real-time', {
        mode: 'real-time',
        loraRank: 2,
        learningRate: 0.001,
        batchSize: 32,
    ...
    it('should initialize with research mode', () => {
      const researchEngine = createPatternLearningEngine('research', {
        mode: 'research',
        loraRank: 16,
        learningRate: 0.002,
        batchSize: 64,
    ...
    it('should initialize with edge mode', () => {
      const edgeEngine = createPatternLearningEngine('edge', {
        mode: 'edge',
        loraRank: 1,
        learningRate: 0.001,
        batchSize: 16,
    ...
    it('should initialize with batch mode', () => {
      const batchEngine = createPatternLearningEngine('batch', {
        mode: 'batch',
        loraRank: 8,
        learningRate: 0.002,
        batchSize: 128,
  ...
  describe('Learning from Trajectories', () => {
    ...
    it('should complete learning under performance target (sub-ms)', async () => {
      const trajectory: Trajectory = {
      ...
      // Note: This is the JS call overhead, actual Pattern is sub-ms
  ...
  describe('Adaptation', () => {
    ...
    it('should handle adaptation with no patterns found', async () => {
      ...
      const freshEngine = new PatternLearningEngine('balanced', modeConfig);
      const context: Context = {
        domain: 'code',
        queryEmbedding: new Float32Array(768).fill(0.5),
      };
  ...
  describe('Mode Switching', () => {
    it('should switch between modes correctly', () => {
      const modes: PatternMode[] = ['real-time', 'balanced', 'research', 'edge', 'batch'];

      modes.forEach(mode => {
        const testEngine = createPatternLearningEngine(mode, {
          mode,
          loraRank: 4,
          learningRate: 0.002,
          batchSize: 32,
