/**
 * Voting Algorithms Tests
 * Comprehensive tests for Raft, Byzantine, and Gossip consensus
 */

import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { RaftVoting, createRaftVoting } from '../src/voting/raft.js';
import { ByzantineVoting, createByzantineVoting } from '../src/voting/byzantine.js';
import { GossipVoting, createGossipVoting } from '../src/voting/gossip.js';
import type { VotingBallot } from '../src/types.js';

describe('Raft Consensus', () => {
  let raft: RaftVoting;

  beforeEach(async () => {
    raft = createRaftVoting('node-1', {
      threshold: 0.66,
      timeoutMs: 5000,
  ...
  describe('Log Replication', () => {
    ...
    it('should propose value as leader', async () => {
      ...
      const raftLeader = createRaftVoting('leader-node', {
        electionTimeoutMinMs: 50,
        electionTimeoutMaxMs: 100,
      });
  ...
  describe('Consensus Process', () => {
    it('should vote on proposal', async () => {
      ...
      const raftLeader = createRaftVoting('leader', {});
      await raftLeader.initialize();
      ...
      if (raftLeader.isLeader()) {
        ...
        const result = await raftLeader.awaitVoting(proposal.id);
    ...
    it('should timeout on consensus', async () => {
      const shortTimeout = createRaftVoting('timeout-node', {
        timeoutMs: 100,
      });
      ...
      await expect(
        shortTimeout.awaitVoting('non-existent-proposal')
...
describe('Byzantine Consensus', () => {
  let byzantine: ByzantineVoting;

  beforeEach(async () => {
    byzantine = createByzantineVoting('node-1', {
      threshold: 0.66,
      timeoutMs: 5000,
      maxFaultyNodes: 1,
    });
  ...
  describe('Three-Phase Commit', () => {
    ...
    it('should reject proposal from non-primary', async () => {
      const nonPrimary = createByzantineVoting('non-primary', {});
  ...
  describe('Fault Tolerance', () => {
    it('should achieve consensus with 2f+1 votes', async () => {
      ...
      const result = await byzantine.awaitVoting(proposal.id);
      expect(result.proposalId).toBe(proposal.id);
    });
  });
});
...
describe('Gossip Consensus', () => {
  let gossip: GossipVoting;

  beforeEach(async () => {
    gossip = createGossipVoting('node-1', {
      threshold: 0.66,
      timeoutMs: 5000,
      fanout: 3,
      gossipIntervalMs: 50,
  ...
  describe('Convergence', () => {
    ...
    it('should achieve eventual consensus', async () => {
      ...
      const result = await gossip.awaitVoting(proposal.id);
    ...
    it('should handle timeout gracefully', async () => {
      const shortGossip = createGossipVoting('timeout-node', {
        timeoutMs: 100,
      ...
      const result = await shortGossip.awaitVoting(proposal.id);
...
describe('Voting Algorithm Comparison', () => {
  it('should handle different voting algorithms', async () => {
    const raft = createRaftVoting('raft-node', {});
    const byzantine = createByzantineVoting('bft-node', {});
    const gossip = createGossipVoting('gossip-node', {});
