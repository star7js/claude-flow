/**
 * V3 Gossip Protocol Voting
 * Eventually consistent voting for large agent groups
 */
...
export interface GossipConfig extends Partial<VotingConfig> {
  fanout?: number;
...
export class GossipVoting extends EventEmitter {
...
export function createGossipVoting(
  nodeId: string,
  config?: GossipConfig
): GossipVoting {
  return new GossipVoting(nodeId, config);
}
